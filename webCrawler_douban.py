...
def parsed_movie(movie):
    ...
    return name, score, num, quotation
...
def parsed_body(body):
    ...
    movie_dict = {}
    movies = []
    start = body.find('<ol class="grid_view">')
    end = body.find('</ol>')

    #删除头结点
    movies = body[start: end].split('<li')
    del movies[0]

    for movie in movies:
        name, score, num, quotation = parsed_movie(movie)
        movie_dict[name] = {
            'score': score,
            'num': num,
            'quotation': quotation,
...
def get_movie_msg(url):
    ...
    url_cp = url
    while page <= 10:
        status_code, headers, body = get(url_cp)
        msg_dict.update(parsed_body(body))
        url_cp = '{}?start={}&filter='.format(url, page*25)
        print(url_cp)
        page += 1
...
def main():
    ...
    for key in movie_dict:
        ...
        i += 1
        print('movie: ', key)
        print('num of Evaluation: ', movie_dict[key]['num'])
        print('score: ', movie_dict[key]['score'])
        print('quotation: ', movie_dict[key]['quotation'])
